simMAEcheck <- function(nsim, islandid, burnin=1000, pc, distr, readLength.pilot=readLength, eset.pilot, usePilot=FALSE, retTxsError=FALSE, genomeDB, mc.cores=1, mc.cores.int=1, verbose=FALSE) {
  ...
  r = readLength.pilot
  f = mean(rep(as.numeric(names(rep1$distr@lenDis)), rep1$distr@lenDis))
  readLength = r
  U <- NULL
  txe <- list()
  pcs <- list()
  disArray <- vector("list", length(f))
  for (i in 1:length(disArray)) disArray[[i]] <- casper:::setfragLength(distr, fragLength=f[i])
  ...
  for (j in 1:length(n)) {
    ...
    pis <- casper:::simPost(islandid=islandid, nsim=nsim, distrs=distr.pilot, genomeDB=genomeDB, pc=pc, readLength=readLength.pilot, mc.cores=mc.cores.int*mc.cores, verbose=verbose)
    ...
    if(mc.cores.int>1) {
      ...
      res <- parallel::mclapply(1:nsim, function(i){
        ...
        pmapped <- (1-casper:::probNonMappable(readLength)) * runif(1,0.6,0.9)  #mapped reads 60%-90% of mappable reads
        N <- round(n[j]*readYield*pmapped)
        sim.pc <-  casper:::simPostPred(islandid=islandid, nreads=N, pis=pis[i,], pc=pc, distrs=distrs, rl=r[j], genomeDB=genomeDB, verbose=verbose)
        if(usePilot) sim.pc$pc <- casper:::mergePCs(pcs=list(sim.pc$pc,pc), genomeDB=genomeDB)
    ...
    if (j==length(n)) { 
      l1 = res
      # nsim will be typically < 10, 
      # so this step should be pretty fast
